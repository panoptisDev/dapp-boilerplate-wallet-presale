...
const BettingPage = () => {
  ...
  const { write: placeBet, isLoading, isSuccess, isError, error } = useWriteContract({
  ...
  return (
    <div className="flex max-w-4xl p-10 mx-auto">
      <div className="p-4 mr-16 rounded-lg bg-light-background dark:bg-dark-background">
        ...
        <br />
        If two predictions are the same, the pot will be split 50/50 between the two players. you can place multiple bets to maximize the chance to win.<br />
        </p>
      </div>
      <div className="flex flex-col items-center w-full">
        ...
        <button className="px-6 py-2 mb-4 text-white transition-transform duration-200 transform bg-green-700 border border-green-300 rounded-full hover:scale-105" onClick={handlePredictionSubmit} disabled={isLoading}>
          {isLoading ? "Submitting..." : "Submit Prediction"}
        </button>
        {isSuccess && <p className="text-green-500">Bet placed successfully!</p>}
        {isError && <p className="text-red-500">Error placing bet: {error?.message}</p>}
      </div>
    </div>
  );
};
