...
contract ForecasterNFT is ERC721, Ownable {
    ...
    struct Forecast {
        ...
        uint256 forecastPrice;
    }

    // Mapping from token ID to Forecast
    mapping(uint256 => Forecast) private _forecasts;

    event ForecastCreated(uint256 indexed tokenId, uint256 creationDate, uint256 forecastPrice);

    constructor(address initialOwner) ERC721("ForecasterNFT", "FCXX") Ownable(initialOwner) {}

    function mintForecast(address recipient, uint256 forecastPrice) public onlyOwner returns (uint256) {
        _tokenIds.increment();
        ...
        emit ForecastCreated(newItemId, creationDate, forecastPrice);

        return newItemId;
    }

    function getForecast(uint256 tokenId) public view returns (uint256, uint256, uint256) {
        require(_exists(tokenId), "ERC721Metadata: Query for nonexistent token");

        Forecast memory forecast = _forecasts[tokenId];
        return (forecast.tokenId, forecast.creationDate, forecast.forecastPrice);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory json = string(abi.encodePacked(
            ...
            '"attributes": [',
            '{"trait_type": "Creation Date", "value": "', uint2str(forecast.creationDate), '"},',
            '{"trait_type": "Forecast Price", "value": "', uint2str(forecast.forecastPrice), '"}',
            ']}'
        ));
