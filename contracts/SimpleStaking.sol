...
contract SimpleStaking is ReentrancyGuard {
    ...
    modifier timestampNotSet() {
        require(!timestampSet, "The timestamp has already been set.");
        _;
    }
    ...
    /// @param _timePeriodInSeconds amount of seconds to add to the initial timestamp i.e. we are essentially creating the minimum staking period here
    ...
    /// @dev Allows the contract owner to reset the timestamp and staking period.
    /// @param _newTimePeriodInSeconds amount of seconds to add to the initial timestamp i.e. we are essentially creating the minimum staking period here
    function resetTimestamp(uint256 _newTimePeriodInSeconds) public onlyOwner {
        initialTimestamp = block.timestamp;
        timePeriod = initialTimestamp.add(_newTimePeriodInSeconds);
    }
    ...
    function transferAccidentallyLockedTokens(IERC20 token, uint256 amount) public onlyOwner nonReentrant {
        ...
        // This function cannot access the official timelocked tokens; just other random ERC20 tokens that may have been accidentally sent here
        require(token != erc20Contract, "Token address cannot be the ERC20 address which was passed into the constructor");
    ...
    /// @dev Returns the staked tokens of a specific user
